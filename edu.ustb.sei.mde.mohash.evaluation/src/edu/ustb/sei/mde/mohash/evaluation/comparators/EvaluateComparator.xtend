...
//		} catch(Exception e) {}
	...
	def static void main(String[] args) {
		...
		if(cat=='ecore') {
			...
			evaluateEcoreModels(models, 100, new File('C:/JavaProjects/git/mohash/edu.ustb.sei.mde.mohash.evaluation/output/ecore_'+set))
		} else {
			val models = modelFolder.listFiles.filter[it.name.endsWith(".xmi")].map[
				// for uml
				it.absolutePath.loadUMLResource
			].toList
			evaluateUMLModels(models, 100, new File('C:/JavaProjects/git/mohash/edu.ustb.sei.mde.mohash.evaluation/output/uml_'+set))
		}
		
	}
	
	
	def static void evaluateEcoreModels(List<EPackage> models, int count, File outputFolder) {
		outputFolder.mkdirs
	...
	def static void evaluateUMLModels(List<Resource> models, int count, File outputFolder) {
		outputFolder.mkdirs
